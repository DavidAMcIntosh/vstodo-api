...
// anonymous async function calling itself.
(async () => {
    ...
    // const user = await User.create({name: "dave"}).save();
    ...
    passport.use(new GitHubStrategy({
        ...
        async (_, __, profile, cb) => {
            ...
            if (user) {
                ...
                console.log('else... displayName = ' + profile.displayName );
                user = await User.create({ name: profile.displayName, githubId: profile.id }).save();
            }
            cb(null, {
                accessToken: jwt.sign({ userId: user.id }, process.env.ACCESS_TOKEN_SECRET, {
                    expiresIn: "1y"
                })
            });
        }
